...
// The React component
const ProductsPerSupplierTableComponent: React.FC<any> = ({ data}) => {
        ...
        <TableBody>
          {data.map((item, index) => (
            <TableRow key={index} className="border-spacing-1 bg-gray-900 text-white">
              <TableCell className="px-4 py-2 text-left">{item.supplier_name}</TableCell>
              <TableCell className="px-4 py-2 text-left">{item.Product_Supplied}</TableCell>
            </TableRow>
          ))}
        </TableBody>
      </TremorTable>
...
export default ProductsPerSupplierTableComponent;
