...
// The React component
const QuoteQuantityProductTableComponent: React.FC<any> = ({ data}) => {
        ...
        <TableBody>
          {data.map((item, index) => (
            <TableRow key={index} className="border-spacing-1 bg-gray-900 text-white">
              <TableCell className="px-4 py-2 text-left">{item.product_name}</TableCell>
              <TableCell className="px-4 py-2 text-left">{item.Quote_Quantity}</TableCell>
            </TableRow>
          ))}
        </TableBody>
      </TremorTable>
...
export default QuoteQuantityProductTableComponent;
