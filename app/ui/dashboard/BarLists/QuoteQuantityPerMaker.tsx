...
type MyComponentProps = {
  data: { maker_name: string, Quote_Quantity: number }[];
};
...
// The React component
export default function QuoteQuantityPerMakerComponentBar({ data }: MyComponentProps) {

  // Transform the data to fit the BarList format
  const transformedData = useMemo(() => {
    return data.map((item, index) => ({
      name: item.maker_name?.toString(),
